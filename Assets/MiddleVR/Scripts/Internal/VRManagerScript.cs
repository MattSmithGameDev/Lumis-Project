...
#if !UNITY_5_6 && !UNITY_2017 && !UNITY_2018 && !UNITY_2019_1 && !UNITY_2019_2 && !UNITY_2019_3
...
#pragma warning restore 618
    ...
    private void _EnableFPSDisplay(bool iEnable)
    {
        ...
        if( this.GetComponent<Canvas>() == null)
        {
            ...
            var watermarkGO = new GameObject("FPS Text");
            watermarkGO.transform.parent = this.transform;

            m_FPSText = watermarkGO.AddComponent<Text>();
            var rct = m_FPSText.rectTransform;

            // Anchor Watermark to center-top
            rct.anchorMin = new Vector2(0.0f, 0.0f);
            rct.anchorMax = new Vector2(0.0f, 0.0f);
            rct.pivot = new Vector2(0.5f, 0.5f);

            // Draw Watermark 100px from the center-top
            rct.anchoredPosition = new Vector3(25.0f, 15.0f, 0.0f);

            m_FPSText.font = Resources.GetBuiltinResource<Font>("Arial.ttf");
            m_FPSText.alignment = TextAnchor.MiddleCenter;
            m_FPSText.fontSize = 15;

            var contentSizeFilter = watermarkGO.AddComponent<ContentSizeFitter>();
            contentSizeFilter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
        }

        Debug.Log("enable: " + m_ShowFPS);
        this.GetComponent<Canvas>().enabled = m_ShowFPS;
    }
